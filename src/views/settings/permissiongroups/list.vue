        ...
        <el-dialog :visible.sync="dialogVisible" :title="dialogType === 'edit' ? 'Edit Role' : 'New Role'">
            <el-form :model="role" label-width="80px" label-position="left">
                ...
                <el-form-item label="Menus">
                    <el-tree ref="tree" :check-strictly="checkStrictly" :auto-expand-parent="true" :data="routesData"
                        :props="defaultProps" show-checkbox node-key="slug" class="permission-tree" />
                </el-form-item>
            </el-form>
...
export default {
    ...
    methods: {
        ...
        generateRoutes(routesInit) {
            const routes = []

            for (let index in routesInit) {
                ...
                if (route.children && Object.keys(route.children).length > 0) {
                    ...
                    let children = this.childrenWithRoutes(route.children)
        ...
        childrenWithRoutes(children) {
            ...
            for (let index in children) {
                ...
                if (child.routes?.length > 0) {
                    let res = this.generateRoutes(child.routes)
                    child.children = res
                }
        ...
        // childrenWithRoutesArr(children) {
        ...
        handleEdit(scope) {
            ...
            this.$nextTick(() => {
                const permissionsInit = JSON.parse(this.role.permissions)
                const permissions = []
                permissionsInit.forEach(route => {
                    permissions.push({ slug: route }, { slug: (route + '/index') })
                })

                this.$refs.tree.setCheckedNodes(permissions)
        ...
        getCheckedRoutes(routes, checkedKeys) {
            ...
            for (const index in routes) {
                ...
                if (checkedKeys.some(child => child == route.slug)) { res.push(route.path) }
